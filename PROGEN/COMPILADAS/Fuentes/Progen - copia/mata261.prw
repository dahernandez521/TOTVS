...
If lContinua
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	aCols   := {}
	cNumSeq := If(Empty(SD3->D3_DOC),SD3->D3_NUMSEQ,'ú')
...
INCLUI := .T. //Variável criada pois quando a função incluir é acionada por MsExecAuto a mesma não é criada
ALTERA := .F. //Variável criada pois quando a função incluir é acionada por MsExecAuto a mesma não é criada
...
INCLUI := .F. //Variável criada pois quando a função é acionada por MsExecAuto a mesma não é criada
ALTERA := .F. //Variável criada pois quando a função é acionada por MsExecAuto a mesma não é criada
...
If lContinua
	...
	aCols   := {}
	aRecnos := {}
	cNumSeq := If(lEstornaIt .Or. Empty(SD3->D3_DOC),SD3->D3_NUMSEQ,'ú')
...
Local lValLote		:= .T.
Local cDocWMS		:= ""
Local lMT260NLOT	:= ExistBlock('MT260NLOT')	//-- Opção para o tipo de SubLote
Local lAtuEti		:= .F.
...
If MultLock("SB2",aLockSB2,1)
	// Verifica se é proveniente de integracao com ACD
...
If !aCols[nLinha,Len(aCols[nLinha])]
	...
	If lAutoma261
		For nPosSX3 := 1 to Len(aCols[nLinha])-1
			...
			If lCampObrig 
				...
				If lEmptyObrg
					lRet := .F.
					//Grava arquivo de LOG do erro da Execauto (necessário para tratamento na ACDV150)
					AutoGRLog("Campo obrigatório vazio: " + aHeader[nPosSX3][2]) //Grava arquivo de LOG
					Exit
				EndIf
			EndIf	
		Next nPosSX3
	...
	If lRet .And. !lWmsSD3
		...
		//Retorna falso caso um produto integre com WMS e o outro não
		If lFnWMSValT
...
If lContinua .And. Empty(cLocal)
	if lAutoma261
		Help(' ',1,'A261VZNNR',,'Não informado depósito origem',1,1)
	Else
		Help(' ',1,'NVAZIO')
	Endif
	lContinua 	:= .F.
...
±±º User Function  ³ IntegDef º Autor ³ Flavio San Miguel    º Data ³  17/12/13   º±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±º User Function  ³Ma261VldDtº Autor ³ Lucas Crevilari      º Data ³  13/07/17   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±º Descricao ³ Funcao para validação da data de fechamento                  º±±
±±ÌÍÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±º Uso       ³ MATA261                                                      º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
