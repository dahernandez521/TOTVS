...
if (isset($_SESSION['ModalCopPro'])) {

    $productos = $_SESSION['ModalCopPro'];
    ...
    foreach ($productos as $row) {
        ...
        $produc = strtoupper(trim($row['productid']));
        ...
        foreach ($_SESSION["productos_obj"] as $key => $Detalle) {
            if ($_SESSION["L_preci"] === $Detalle["code_lp"]) {
                $fail = 1;
                $ProdFin = trim(strtoupper(trim($row['productid'])));
                    ...
                    if (isset($_SESSION['RCruz'])) {
                        foreach ($_SESSION['RCruz'] as $ref) {
                            if (trim($ref['cliente'] === trim($_SESSION['Cliente']))) {
                                if (trim(strtoupper($ref['codecli'])) === trim(strtoupper($row['productid']))) {
                                    $produc = $ref['codepro'];
                                    $ProdFin = $ref['codecli'];
                                    break;
                                }
                        ...
                        '             
                            ...
                            <div class="modal-dialog  modal-lg">
                                <div class="modal-content">
                                    ...
                                    <div class="col-sm-7">
                                    <input type="text" required disabled value="' . $podes . '" id="totDescuento' . $contador . '" autocomplete="off" class="form-control desc-tot">
                                    </div>           </div>           </div>           </div>                                      
                                    </div>
