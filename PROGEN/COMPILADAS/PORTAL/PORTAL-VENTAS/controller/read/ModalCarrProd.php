...
if (isset($_SESSION['ModalCarrPro'])) {

    $productos = $_SESSION['ModalCarrPro'];
    ...
    foreach ($productos as $row) {
        ...
        $produc = strtoupper(trim($row['Producto']));
        ...
        foreach ($_SESSION["productos_obj"] as $key => $Detalle) {
            if ($_SESSION["L_preci"] === $Detalle["code_lp"]) {
                $fail = 1;
                $ProdFin = trim(strtoupper(trim($row['Producto'])));
                    ...
                    if (isset($_SESSION['RCruz'])) {
                        foreach ($_SESSION['RCruz'] as $ref) {
                            if (trim($ref['cliente'] === trim($_SESSION['Cliente']))) {
                                if (trim(strtoupper($ref['codecli'])) === trim(strtoupper($row['Producto']))) {
                                    $produc = $ref['codepro'];
                                    $ProdFin = $ref['codecli'];
                                    break;
                                }
                    ...
                    if (
                        ...
                        trim($_SESSION['Cliente']) == "901126410"
                                    ...
                                    $Impiva = '<tr>
                                        ...
                                        <td>
                                        <input style="border:none;text-align:right;" type="number" required disabled value="' . number_format($ValIva, 2, '.', ',') . '" id="TotImp' . $contador . '" autocomplete="off" class="form-control TotImpuIva">
                                        </td>
                                        </tr>';
